(function(){function range(start,stop,step){if(arguments.length<=1){stop=start||0;start=0}step=arguments[2]||1;var length=Math.max(Math.ceil((stop-start)/step),0);var idx=0;var range=new Array(length);while(idx<length){range[idx++]=start;start+=step}return range}function len(obj){if(obj instanceof Array||typeof obj==="string")return obj.length;else{var count=0;for(var i in obj){if(obj.hasOwnProperty(i))count++}return count}}function _$rapyd$_in(val,arr){if(arr instanceof Array||typeof arr==="string")return arr.indexOf(val)!=-1;else{for(i in arr){if(arr.hasOwnProperty(i)&&i===val)return true}return false}}function dir(item){var arr=[];for(var i in item){arr.push(i)}return arr}function _$rapyd$_extends(child,parent){child.prototype=new parent;child.prototype.constructor=child}function _$rapyd$_print(){var args,output;args=[].slice.call(arguments,0);output=JSON.stringify(args);if("console"in window)console.log(output.substr(1,output.length-2))}var JSON,str,PREFIX_LENGTH,STOP_WORDS;JSON=JSON||{};if(!JSON.stringify){
	...
	Array.prototype.filter = function(filterfun, thisArg) {
        ...
        ;return keys}}else{dict.keys=function(hash){return Object.getOwnPropertyNames(hash)}}dict.values=function(hash){var vals,key;vals=[];var _$rapyd$_Iter3=dict.keys(hash);for(var _$rapyd$_Index3=0;_$rapyd$_Index3<_$rapyd$_Iter3.length;_$rapyd$_Index3++){key=_$rapyd$_Iter3[_$rapyd$_Index3];vals.append(hash[key])}return vals};dict.items=function(hash){var items,key;items=[];var _$rapyd$_Iter4=dict.keys(hash);for(var _$rapyd$_Index4=0;_$rapyd$_Index4<_$rapyd$_Iter4.length;_$rapyd$_Index4++){key=_$rapyd$_Iter4[_$rapyd$_Index4];items.append([key,hash[key]])}return items};dict.copy=dict;dict.clear=function(hash){var key;var _$rapyd$_Iter5=dict.keys(hash);for(var _$rapyd$_Index5=0;_$rapyd$_Index5<_$rapyd$_Iter5.length;_$rapyd$_Index5++){key=_$rapyd$_Iter5[_$rapyd$_Index5];delete hash[key]}};PREFIX_LENGTH=2;STOP_WORDS=null;$.get("../stop_words.txt",function(data){STOP_WORDS=data.trim().split(new RegExp("\\s+"))});function load_word_lists(){var texts,word_lists,word_list,text;texts=[$("#text1").val(),$("#text2").val(),$("#text3").val()];word_lists=[];var _$rapyd$_Iter6=texts;for(var _$rapyd$_Index6=0;_$rapyd$_Index6<_$rapyd$_Iter6.length;_$rapyd$_Index6++){text=_$rapyd$_Iter6[_$rapyd$_Index6];word_list=text.trim().split(new RegExp("\\s+"));if(len(word_list)>PREFIX_LENGTH){word_lists.append(word_list)}}return word_lists}function load_num_words(){return parseInt($("#num-words").val())}function get_words(word_lists,ratios){var R,m,counts,result,wl;"\n    Merge the list of texts according to the given ratios and\n    return the resulting text split into words.\n    ";R=sum(ratios);m=Math.min.apply(Math,[].concat((function(){var _$rapyd$_Iter=word_lists,_$rapyd$_Result=[],wl;for(var _$rapyd$_Index=0;_$rapyd$_Index<_$rapyd$_Iter.length;_$rapyd$_Index++){wl=_$rapyd$_Iter[_$rapyd$_Index];_$rapyd$_Result.push(len(wl))}return _$rapyd$_Result})()));counts=(function(){var _$rapyd$_Iter=ratios,_$rapyd$_Result=[],r;for(var _$rapyd$_Index=0;_$rapyd$_Index<_$rapyd$_Iter.length;_$rapyd$_Index++){r=_$rapyd$_Iter[_$rapyd$_Index];_$rapyd$_Result.push(Math.max(PREFIX_LENGTH,Math.ceil(m*r/R)))}return _$rapyd$_Result})();word_lists=(function(){var _$rapyd$_Iter=range(len(word_lists)),_$rapyd$_Result=[],i;for(var _$rapyd$_Index=0;_$rapyd$_Index<_$rapyd$_Iter.length;_$rapyd$_Index++){i=_$rapyd$_Iter[_$rapyd$_Index];_$rapyd$_Result.push(word_lists[i].slice(0,counts[i]))}return _$rapyd$_Result})();result=[];var _$rapyd$_Iter7=word_lists;for(var _$rapyd$_Index7=0;_$rapyd$_Index7<_$rapyd$_Iter7.length;_$rapyd$_Index7++){wl=_$rapyd$_Iter7[_$rapyd$_Index7];result.extend(wl)}return result}function pack(items){"\n    Because RapydScript doesn't have ordered items :-(, \n    use item1##item2 to simulate one \n    ";return items.join("##")}function unpack(items){return items.split("##")}function get_markov_analysis(words,prefix_length){if(typeof prefix_length==="undefined")prefix_length=PREFIX_LENGTH;var result,prefix,suffix,i;"\n    Return a Markov analysis of the list of strings ``words``.\n    The output format is a dictionary with structure: \n    a tuple of contiguous words in ``words`` of length ``prefix_length`` \n    (a prefix) -> a list of all individual words in ``words`` that occur \n    after that prefix.\n    ";result={};for(i=0;i<len(words)-prefix_length;i++){prefix=pack(words.slice(i,i+prefix_length));suffix=words[i+prefix_length];if(_$rapyd$_in(prefix,result)){result[prefix].append(suffix)}else{result[prefix]=[suffix]}}return result}function shuffle(a){var i,_$rapyd$_Unpack,m;"\n    Randomly shuffle the given list and return the result.\n    Uses the Fisher–Yates shuffle, which is explained well at \n    http:#bost.ocks.org/mike/shuffle/ .\n    ";m=len(a)-1;while(m){i=Math.floor(Math.random()*m);_$rapyd$_Unpack=[a[i],a[m]];a[m]=_$rapyd$_Unpack[0];a[i]=_$rapyd$_Unpack[1];m-=1}return a}function choose(a){var i;"\n    Return a random element from the given list\n    ";i=Math.floor(Math.random()*len(a));return a[i]}function get_mix(words,num_words,prefix_length){if(typeof prefix_length==="undefined")prefix_length=2;var d,prefixes,result,prefix,s,suffix,i;"\n    Return a list of ``num_words`` random words from \n    the given list of words. \n    Do this by shuffling ``d = get_markov_analysis(words, prefix_length)``,\n    and traversing ``d`` until ``num_words`` random words have been \n    generated.\n    ";d=get_markov_analysis(words,prefix_length);if(!len(d)){return[]}prefixes=shuffle(dict.keys(d));result=unpack(prefixes[0]);i=0;while(i<num_words){prefix=pack(result.slice(i,i+prefix_length));while(!(_$rapyd$_in(prefix,d))){prefix=choose(prefixes)}s=d[prefix];suffix=choose(s);result.append(suffix);i+=1}return result}function dump_mix(mix){$("#mix").val(mix)}function load_mix(){return $("#mix").val()}function load_poem(){return $("#poem").val()}function remove_punctuation(string){return string.replace(new RegExp("[\\.,-\\/#!$%\\^&\\*;:{}=\\-_`~()@\\+\\?><\\[\\]\\+]","g"),"")}function validate(){var mix,mix_words,mix_stems,poem_words,invalid_words,w,word;mix=remove_punctuation(load_mix().trim()).split(new RegExp("\\s+"));mix_words=(function(){var _$rapyd$_Iter=mix,_$rapyd$_Result=[],word;for(var _$rapyd$_Index=0;_$rapyd$_Index<_$rapyd$_Iter.length;_$rapyd$_Index++){word=_$rapyd$_Iter[_$rapyd$_Index];if(!(_$rapyd$_in(word,STOP_WORDS))){_$rapyd$_Result.push(word.toLowerCase())}}return _$rapyd$_Result})();mix_stems=(function(){var _$rapyd$_Iter=mix_words,_$rapyd$_Result=[],word;for(var _$rapyd$_Index=0;_$rapyd$_Index<_$rapyd$_Iter.length;_$rapyd$_Index++){word=_$rapyd$_Iter[_$rapyd$_Index];_$rapyd$_Result.push(stemmer(word))}return _$rapyd$_Result})();poem_words=remove_punctuation(load_poem().trim()).split(new RegExp("\\s+"));invalid_words=[];var _$rapyd$_Iter8=poem_words;for(var _$rapyd$_Index8=0;_$rapyd$_Index8<_$rapyd$_Iter8.length;_$rapyd$_Index8++){word=_$rapyd$_Iter8[_$rapyd$_Index8];w=remove_punctuation(word.toLowerCase());if(_$rapyd$_in(w,STOP_WORDS)){continue}if(!(_$rapyd$_in(stemmer(w),mix_stems))&&!(_$rapyd$_in(word,invalid_words))){invalid_words.append(word)}}return invalid_words}function dump_validation(report){$("#validation-report").html(report)}$("#make-mix").click(function(){$("#text1").effect("shake",{"times":3},800);$("#text2").effect("shake",{"times":3},1e3);$("#text3").effect("shake",{"times":3},1200);setTimeout(function(){var word_lists,ratios,words,num_words,mix;$("#mix").effect("shake",{"times":1},800);_$rapyd$_print("Hello!");word_lists=load_word_lists();ratios=(function(){var _$rapyd$_Iter=len(word_lists),_$rapyd$_Result=[],i;for(var _$rapyd$_Index=0;_$rapyd$_Index<_$rapyd$_Iter.length;_$rapyd$_Index++){i=_$rapyd$_Iter[_$rapyd$_Index];_$rapyd$_Result.push(1)}return _$rapyd$_Result})();words=get_words(word_lists,ratios);_$rapyd$_print("source num words",len(words));num_words=load_num_words();_$rapyd$_print("mix num words",num_words);mix=get_mix(words,num_words);dump_mix(mix.join(" "))},1e3)});$("#validate").click(function(){var invalid_words,report;invalid_words=validate();report="<p>The following words in your poem appear to be invalid:</p>"+"<textarea class=\"short\">"+invalid_words.join(", ")+"</textarea>";dump_validation(report)})})();